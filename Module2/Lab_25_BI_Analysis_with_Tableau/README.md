...
Month-to-Month is the most popular contract for electronic check and mailed check, especially for paperless billing.
...
When we add Dependents and Partners on top of Gender, we can see that the reverse trend for Gender during the 3 first months is mostly true for Women with Partners and Dependents: one possible explanation is that mothers, rather than fathers, subscribe for their children, and this subscription occurs only once by household, explaining the gap between the 2 genders. 
...
Seniors are charged $20 more in average than non-seniors, with a variation of this gap over time.
